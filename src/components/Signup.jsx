        ...
        <div className="right w-[67%] h-full pr-5 pt-5 flex-col">
          ...
          <div className="formSignup flex flex-col gap-10 w-[80%] text-left">
            ...
            <div className="2inputs flex flex-row pl-40 pb-0 justify-between w-full">
              <h2 className='font-semibold w-1/2'>Name <br /> <input className="font-normal p-2 text-normal w-1/2 bg-slate-200 rounded-lg" type="text" name="name" value={formData.name} onChange={handleChange} /></h2>
              <h2 className='font-semibold w-1/2'>Username <br />  <input className="font-normal p-2 text-normal w-1/2  bg-slate-200 rounded-lg" type="text" name="username" value={formData.username} onChange={handleChange} /></h2>
            </div>
            <div className="email_password flex flex-col gap-10 w-full pl-40 pb-0">
              <div className="email">
                <h2 className='font-semibold w-full'>Email<br /> <input className="font-normal p-2 text-normal w-full  bg-slate-200 rounded-lg" type="text" name="email" value={formData.email} onChange={handleChange} /></h2>
              </div>
              <div className="Password">
                <h2 className='font-semibold w-full'>Password<br /> <input placeholder='6+ Characters' className="font-normal  text-normal w-full p-2 bg-slate-200 rounded-lg" type="password" name="password" value={formData.password} onChange={handleChange} /></h2>
              </div>
              ...
              <div className='Button'>
                <button style={{ backgroundColor: "#EA4B8B" }} onClick={movenextPage} className=" text-white font-bold py-2 px-4 rounded-lg">
                  Create Account
                </button>
              </div>
